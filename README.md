<h1 align="center">Hi 👋, I'm Birchove</h1>
<h3 align="center">CS Sophomore | Algorithms | Internship Seeker | AI Explorer</h3>

<p align="center">
  <a href="https://github.com/Birchove">
    <!-- Typing 动画已更新为你最新的状态和关键词 -->
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&pause=1000&color=3B82F6&center=true&vCenter=true&width=500&lines=CS+Sophomore+@+SEU;Looking+for+Internships;Coding+in+Python+%26+C%2B%2B;Exploring+AI+Workflows" alt="Typing SVG" />
  </a>
</p>

<!-- 访客与社交链接徽章 -->
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Birchove&label=Profile%20Views&color=0e75b6&style=flat-square" alt="Visitor Badge" />
  <a href="mailto:zhaoxuanwang666@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://www.zhihu.com/people/meng-dong-de-xiao-bai-yi-mei" target="_blank">
    <img src="https://img.shields.io/badge/Zhihu-0084FF?style=flat-square&logo=zhihu&logoColor=white" alt="Zhihu" />
  </a>
</p>

---

### 👨‍💻 About Me

- 🎓 I’m a **CS Sophomore at SEU (Southeast University)**, currently actively looking for an **internship**.
- 🔭 My latest project is **[AIWolfGame](https://github.com/Birchove/AIWolfGame)**. Previously, I participated in a GameJam and developed an ink-wash card game called **[Feibai](https://github.com/Birchove/Feibai-GameJam)**.
- 🌱 I’m currently diving deep into **AI technologies** and core CS curriculum (**408**).
- 🎯 Future Goal: I aim to work in **AI-related roles** directly after graduation (not planning on a Master's degree for now).
- 💬 Let's exchange ideas on: **Python, C++, Basic Algorithms, AI usage tips/experiences, and interesting projects**.
- 📫 How to reach me: **[zhaoxuanwang666@gmail.com](mailto:zhaoxuanwang666@gmail.com)** or connect on **[Zhihu](https://www.zhihu.com/people/meng-dong-de-xiao-bai-yi-mei)**.
- ⚡ Fun fact: I might be a "novice" in many areas, but I love pushing the boundaries of AI-assisted development!

---

### 🚀 Featured Projects

<!-- 采用原生 Markdown 列表展示项目，保证在任何网络环境下 100% 稳定显示，这对于找实习非常重要 -->

* 🐺 **[AIWolfGame](https://github.com/Birchove/AIWolfGame)** <br> My latest project focusing on multi-agent AI game simulation.
* 🃏 **[Feibai-GameJam](https://github.com/Birchove/Feibai-GameJam)** <br> An ink-wash wuxia roguelike card game developed during AI GameJam.
* 🔍 **[RAG-ES-Pipeline](https://github.com/Birchove/RAG-ES-Pipeline)** <br> A retrieval-augmented generation pipeline using Elasticsearch.
* 🧠 **[multimodal-rag-api](https://github.com/Birchove/multimodal-rag-api)** <br> An API for multimodal RAG processing and application.

---

### 🛠️ Languages and Tools

<p align="left">
  <!-- Core Skills: 修复了原有的 SQL 报错问题 -->
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=python,cpp,cs,qt,linux,github,vscode&perline=10" alt="Core Skills" />
  </a>
  <br><br>
  <!-- AI & Dev Tools: 采用定制徽章完美展示最新 AI 工具 -->
  <img src="https://img.shields.io/badge/Cursor-000000?style=for-the-badge&logo=cursor&logoColor=white" alt="Cursor" />
  <img src="https://img.shields.io/badge/Claude_Code-D97757?style=for-the-badge&logo=anthropic&logoColor=white" alt="Claude Code" />
  <img src="https://img.shields.io/badge/OpenAI_Codex-412991?style=for-the-badge&logo=openai&logoColor=white" alt="OpenAI Codex" />
</p>

---

### 📊 GitHub Stats & Activity

<div align="center">
  <img src="https://github-readme-stats-xi-ruddy-88.vercel.app/api?username=Birchove&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="48%" alt="GitHub Stats" />
  <img src="https://github-readme-stats-xi-ruddy-88.vercel.app/api/top-langs/?username=Birchove&theme=tokyonight&hide_border=true&layout=compact" width="48%" alt="Top Langs" />
</div>

<br>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Birchove&theme=tokyo-night&hide_border=true" width="100%" alt="Activity Graph" />
</div>

---
<p align="center">
  <i>Let's connect and build something awesome!</i>
</p>
